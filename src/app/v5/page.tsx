...
function HomeScreen({ onStart }: { onStart: () => void }) {
  return (
    <div className="flex min-h-screen flex-col items-center justify-center px-4">
      ...
      >
          {/* Claude Logo — use the actual uploaded image with rounded corners, no white bg visible */}
          <img
            src="/claude-logo.png"
            alt="Claude Code"
            width={44}
          ...
          <p style={{ color: "var(--v5-text-secondary)", fontSize: "10px", lineHeight: "1.6" }}>
            Made with ♥ by{" "}
            <a href={LINKEDIN_URL} target="_blank" rel="noopener noreferrer"
              style={{ color: "var(--v5-accent)", fontWeight: 600, textDecoration: "none" }}>Krishna Goyal</a>
          </p>
        </motion.div>
    </div>
  );
...
// ────────────────────────────────────────────────────────────
// TOP BAR (name + heart, top-right corner)
// ────────────────────────────────────────────────────────────
function V5Credit() {
  return (
    <div className="py-5 flex items-center justify-center">
      ...
      >
        <span style={{ color: "var(--v5-text-tertiary)", fontSize: "11px" }}>Made with</span>
        <span style={{ color: "var(--v5-accent)", fontSize: "12px" }}>♥</span>
        <span style={{ color: "var(--v5-text-tertiary)", fontSize: "11px" }}>by</span>
        <span style={{ color: "var(--v5-accent)", fontSize: "11px", fontWeight: 600 }}>Krishna Goyal</span>
      </a>
    </div>
  );
}
...
function FeedbackBox({ show, correct, text }: { show: boolean; correct: boolean | null; text: string }) {
  ...
  return (
    <motion.div
      ...
      style={{
        background: correct ? "rgba(134,239,172,0.08)" : "rgba(252,165,165,0.08)",
        border: `1px solid ${correct ? "rgba(134,239,172,0.2)" : "rgba(252,165,165,0.2)"}`,
...
function TruthOrMythCard({ challenge, onAnswer }: { challenge: TruthOrMythChallenge; onAnswer: (c: boolean) => void }) {
  ...
  const btnStyle = (pick: boolean) => {
    ...
    if (answered === null) {
      return {
        background: pick ? "rgba(134,239,172,0.06)" : "rgba(252,165,165,0.06)",
        border: `1px solid ${pick ? "rgba(134,239,172,0.25)" : "rgba(252,165,165,0.25)"}`,
        color: pick ? "var(--v5-correct)" : "var(--v5-wrong)",
      };
    }
    if (isThis) {
      return {
        background: isCorrect ? "rgba(134,239,172,0.15)" : "rgba(252,165,165,0.15)",
...
function ThisOrThatCard({ challenge, onAnswer }: { challenge: ThisOrThatChallenge; onAnswer: (c: boolean) => void }) {
  ...
  const optStyle = (opt: "A" | "B") => {
    ...
    if (opt === challenge.correct) return { background: "rgba(134,239,172,0.08)", border: "1px solid var(--v5-correct)", color: "var(--v5-correct)" };
    if (opt === picked) return { background: "rgba(252,165,165,0.08)", border: "1px solid var(--v5-wrong)", color: "var(--v5-wrong)" };
    return { background: "transparent", border: "1px solid var(--v5-border)", color: "var(--v5-text-tertiary)", opacity: 0.3 };
  };
...
function QuickPickCard({ challenge, onAnswer }: { challenge: QuickPickChallenge; onAnswer: (c: boolean) => void }) {
  ...
  const optStyle = (opt: string) => {
    if (picked === null) return { background: "transparent", border: "1px solid var(--v5-border)", color: "var(--v5-text)" };
    if (opt === challenge.blank) return { background: "rgba(134,239,172,0.08)", border: "1px solid var(--v5-correct)", color: "var(--v5-correct)" };
    if (opt === picked) return { background: "rgba(252,165,165,0.08)", border: "1px solid var(--v5-wrong)", color: "var(--v5-wrong)" };
    return { background: "transparent", border: "1px solid var(--v5-border)", color: "var(--v5-text-tertiary)", opacity: 0.3 };
  };
...
function SpeedPickCard({ challenge, onAnswer }: { challenge: SpeedPickChallenge; onAnswer: (c: boolean) => void }) {
  ...
  const chipStyle = (item: string) => {
    ...
    if (isRight && isSel) return { background: "rgba(134,239,172,0.1)", border: "1px solid var(--v5-correct)", color: "var(--v5-correct)" };
    if (isRight && !isSel) return { background: "rgba(134,239,172,0.04)", border: "1px solid rgba(134,239,172,0.2)", color: "rgba(134,239,172,0.5)" };
    if (!isRight && isSel) return { background: "rgba(252,165,165,0.1)", border: "1px solid var(--v5-wrong)", color: "var(--v5-wrong)" };
  ...
  return (
    <div className="flex flex-col items-center gap-5 w-full">
      <div className="w-full flex items-center gap-3">
        <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: "var(--v5-bg-surface)" }}>
          <motion.div
...
function OddOneOutCard({ challenge, onAnswer }: { challenge: OddOneOutChallenge; onAnswer: (c: boolean) => void }) {
  ...
  const itemStyle = (item: string) => {
    if (picked === null) return { background: "transparent", border: "1px solid var(--v5-border)", color: "var(--v5-text)" };
    if (item === challenge.oddItem) return { background: "rgba(134,239,172,0.08)", border: "1px solid var(--v5-correct)", color: "var(--v5-correct)" };
    if (item === picked) return { background: "rgba(252,165,165,0.08)", border: "1px solid var(--v5-wrong)", color: "var(--v5-wrong)" };
    return { background: "transparent", border: "1px solid var(--v5-border)", color: "var(--v5-text-tertiary)", opacity: 0.3 };
  };
...
export default function V5Page() {
  ...
  const handleNext = useCallback(() => {
    if (currentIndex < total - 1) {
      ...
      router.push(`/v5/results?${encoded}`);
    }
      ...
      {answered && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-6">
          <button onClick={handleNext}
            ...
            style={{ background: "var(--v5-accent)", color: "var(--v5-bg)" }}>
