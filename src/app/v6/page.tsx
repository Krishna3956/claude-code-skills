...
function HomeScreen({ onStart }: { onStart: () => void }) {
  return (
    <div className="flex min-h-screen flex-col items-center justify-center px-4">
      ...
      {/* Credit: top-right on desktop, hidden on mobile when PH badge shows */}
      <div className="fixed top-0 left-0 right-0 z-50 flex justify-center sm:justify-end px-4 py-3 pointer-events-none" style={{ top: "60px" }}>
        <a href={LINKEDIN_URL} target="_blank" rel="noopener noreferrer"
          className="flex items-center gap-1.5 transition-opacity hover:opacity-80 pointer-events-auto sm:mt-0"
...
export default function V6Page() {
  ...
  const handleNext = useCallback(() => {
    if (currentIndex < total - 1) {
      ...
      router.push(`/v6/results?${encoded}`);
    }
