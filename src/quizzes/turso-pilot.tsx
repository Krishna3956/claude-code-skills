import type { QuizConfig } from "@/components/quiz/types";

export const tursoPilotConfig: QuizConfig = {
  slug: "turso-pilot",
  toolName: "Turso",
  tagline: "6 rounds. ~2 min. No signup required.",
  subtitle: "Just you vs. Turso knowledge challenge.",
  sansFont: "inter",
  serifFont: "instrument-serif",
  hideNavbar: true,
  hidePopup: true,
  hideScorecardBranding: true,
  customDownloadName: "my-turso-scorecard",
  accentColor: "#4FF8D2",
  accentColorDim: "#3ADBB8",
  accentColorSubtle: "rgba(79,248,210,0.10)",
  bgColor: "linear-gradient(145deg, #0A0E12 0%, #0F1419 55%, #141C22 100%)",
  bgElevated: "#161E26",
  bgSurface: "#1A242E",
  bgSurfaceLight: "#202C38",
  textColor: "#E8F0F5",
  textSecondary: "#A8BCC8",
  textTertiary: "#7A94A5",
  borderColor: "#283844",
  borderSubtle: "#202C38",
  scorecardBg: "#080C10",
  scorecardAccentColor: "#7AFCE5",
  scorecardDivider: "#1E3A35",
  scorecardLabelColor: "#A0C8C0",
  scorecardGridColor: "#1E3A35",
  navbarTheme: "dark",
  ctaTextColor: "#000000",
  logo: (<img src="/logos/turso.ico" alt="Turso" width={44} height={44} style={{ objectFit: "contain", borderRadius: 12, padding: 6, backgroundColor: "#FFFFFF", border: "1px solid #283844" }} />),
  scorecardLogo: (<img src="/logos/turso.ico" alt="Turso" width={42} height={42} style={{ borderRadius: 12, objectFit: "contain", padding: 6, backgroundColor: "#FFFFFF", border: "1px solid #283844" }} />),
  analyticsPrefix: "turso_pilot",
  dimensionLabels: {
    memory: "Core Fundamentals",
    orchestration: "Data Architecture",
    automation: "CLI & Tooling",
    extensibility: "AI & Vectors",
    workflows: "Sync & Replicas",
    prompting: "Query & Search",
    bestPractices: "Security & Ops",
  },
  archetypes: [
    { title: "Turso Grandmaster", emoji: "🏆", description: "You know Turso inside and out, from embedded replicas to BYOK encryption.", minScore: 90, maxScore: 100 },
    { title: "Database Architect", emoji: "🧠", description: "Deep understanding of Turso's distributed architecture and feature set.", minScore: 80, maxScore: 89 },
    { title: "libSQL Power User", emoji: "⚡", description: "Strong grasp of Turso's core features and practical workflows.", minScore: 70, maxScore: 79 },
    { title: "Cloud Builder", emoji: "🛠️", description: "Solid knowledge of Turso Cloud with room to explore advanced features.", minScore: 60, maxScore: 69 },
    { title: "SDK Practitioner", emoji: "📈", description: "Good working knowledge. More time with branching and vectors will level you up.", minScore: 50, maxScore: 59 },
    { title: "Explorer", emoji: "🌱", description: "You know the basics. Dive deeper into embedded replicas and the CLI.", minScore: 40, maxScore: 49 },
    { title: "Getting Started", emoji: "🚀", description: "Great first step. The Turso docs at docs.turso.tech are your best next move.", minScore: 0, maxScore: 39 },
  ],
  challenges: [
    { type: "truth_or_myth", id: 1, dimension: "memory", statement: "Turso is built on libSQL, an open-source fork of SQLite", isTrue: true, explanation: "Turso is powered by libSQL, which is an open contribution fork of SQLite, maintaining full SQLite compatibility." },
    { type: "truth_or_myth", id: 2, dimension: "workflows", statement: "Embedded replicas in Turso always send reads to the remote primary database", isTrue: false, explanation: "Reads are always served from the local replica. Only writes are sent to the remote primary database by default." },
    { type: "truth_or_myth", id: 3, dimension: "extensibility", statement: "Vector search in Turso requires a separate plugin or extension to be installed", isTrue: false, explanation: "Vector similarity search is built into Turso and libSQL as a native feature. No plugins needed." },
    { type: "this_or_that", id: 4, dimension: "orchestration", scenario: "You want to create an isolated copy of your production database for testing. Best approach?", optionA: "Manually export and re-import the entire database", optionB: "Use Turso branching with --from-db to create a copy-on-write branch", correct: "B", explanation: "Turso branching creates a new database from an existing one using copy-on-write, making it fast and ideal for testing." },
    { type: "this_or_that", id: 5, dimension: "workflows", scenario: "Your app needs to work offline but sync when connectivity returns. Which Turso feature?", optionA: "Embedded replicas with offline mode set to true", optionB: "Point-in-time recovery", correct: "A", explanation: "Embedded replicas support an offline config option that enables local writes when disconnected, syncing back when online." },
    { type: "quick_pick", id: 6, dimension: "automation", scenario: "What CLI command creates a new Turso database branch from an existing database?", blank: "turso db create [name] --from-db [source]", options: ["turso branch new [name] --source [db]", "turso db create [name] --from-db [source]", "turso db fork [name] --copy [source]"], explanation: "The turso db create command with the --from-db flag creates a branch from an existing database." },
    { type: "quick_pick", id: 7, dimension: "extensibility", scenario: "Which function performs cosine similarity search on vectors in Turso?", blank: "vector_distance_cos", options: ["cosine_search", "similarity_match", "vector_distance_cos"], explanation: "vector_distance_cos is the built-in function for cosine distance calculations on vector columns in Turso." },
    { type: "speed_pick", id: 8, dimension: "automation", prompt: "Tap every official Turso SDK language!", correctItems: ["TypeScript / JS", "Rust", "Go", "Python"], wrongItems: ["Haskell", "Erlang", "COBOL", "Fortran"], timeLimit: 12, explanation: "Turso provides official SDKs for TypeScript/JS, Rust, Go, Python, plus Android (Kotlin), Swift, PHP, and Ruby." },
    { type: "odd_one_out", id: 9, dimension: "orchestration", prompt: "Which is NOT a Turso Cloud feature?", items: ["Embedded Replicas", "Database Branching", "Point-in-Time Recovery", "Automatic Schema Migration"], oddItem: "Automatic Schema Migration", explanation: "Turso Cloud offers embedded replicas, branching, and PITR. Schema migrations must be handled manually or with external tools." },
    { type: "odd_one_out", id: 10, dimension: "memory", prompt: "Which encryption cipher is NOT supported by Turso's BYOK encryption?", items: ["AES-GCM", "ChaCha20-Poly1305", "AEGIS", "RSA-OAEP"], oddItem: "RSA-OAEP", explanation: "Turso BYOK encryption supports AES-GCM, ChaCha20-Poly1305, and AEGIS ciphers. RSA-OAEP is not a supported cipher." },
  ],
  rounds: [
    { name: "Truth or Myth", icon: "?", ids: [1, 2, 3], tagline: "Real feature or total BS?" },
    { name: "This or That", icon: "vs", ids: [4, 5], tagline: "Pick the smarter move" },
    { name: "Quick Pick", icon: ">>", ids: [6, 7], tagline: "Name that command" },
    { name: "Speed Round", icon: "::", ids: [8], tagline: "Tap fast, think faster" },
    { name: "Odd One Out", icon: "//", ids: [9], tagline: "Spot the fake" },
    { name: "Final Boss", icon: "*", ids: [10], tagline: "One shot. Expert level." },
  ],
};
